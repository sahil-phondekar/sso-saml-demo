...
import { generateSaml2ResponseSignatureNode } from "../utilities/generate-saml2-response-signature-node";
import { generateSignedSAML2AssertionNode } from "../utilities/generate-signed-saml2-assertion-node";

export const generateCompleteSignedSaml2Response = ({ data, keys }) => {
  const {
  ...
  const saml2SignatureNodeXml = generateSaml2ResponseSignatureNode({
    data,
    keys,
  });
  ...
  const saml2XML = xmlbuilder2.create(options, {
    "saml2p:Response": { ...saml2ResponseAttributes },
  });
