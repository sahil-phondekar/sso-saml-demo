...
exports.generateCompleteSignedSaml2Response = void 0;
...
var _generateSaml2ResponseSignatureNode = require("../utilities/generate-saml2-response-signature-node");
var _generateSignedSaml2AssertionNode = require("../utilities/generate-signed-saml2-assertion-node");
...
const generateCompleteSignedSaml2Response = ({
  data,
  keys
}) => {
  const {
  ...
  const saml2IssuerNodeXml = xmlbuilder2.create(saml2IssuerNode);
  const saml2SignatureNodeXml = (0, _generateSaml2ResponseSignatureNode.generateSaml2ResponseSignatureNode)({
    data,
    keys
  });
  const saml2StatusNodeXml = xmlbuilder2.create(saml2StatusNode);
  const saml2AssertionNodeXml = xmlbuilder2.create(saml2AssertionNode);
  const signedAssertion = (0, _generateSignedSaml2AssertionNode.generateSignedSAML2AssertionNode)({
    assertionNodeXML: saml2AssertionNodeXml,
    keys
  });
  const saml2XML = xmlbuilder2.create(options, {
    "saml2p:Response": _objectSpread({}, saml2ResponseAttributes)
  });
  saml2XML.root().ele(saml2IssuerNodeXml.toString());
  saml2XML.root().ele(saml2SignatureNodeXml.toString());
  saml2XML.root().ele(saml2StatusNodeXml.toString());
  saml2XML.root().ele(signedAssertion.toString());
  return saml2XML;
};
exports.generateCompleteSignedSaml2Response = generateCompleteSignedSaml2Response;